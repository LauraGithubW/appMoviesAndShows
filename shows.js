...
async function getShows() {
  ...
  let res = await fetch(
      `https://api.themoviedb.org/3/tv/popular?api_key=a4e65b6f99641fe9c6618d74e26d0b8a&language=en-US&page=${page}`
    ),
    json = await res.json();
  ...
  for (let i = 0; i < json.results.length; i++) {
    try {
      ...
      $section.innerHTML += `
        ...
        <img src="https://image.tmdb.org/t/p/original${shows.backdrop_path}" alt="${shows.name}" class="movie-poster">
        <h3 class="movie-title" ><b>Título</b>: ${shows.name}</h3>
        <figcaption class="movie-figcaption"><b>Descripción: </b>${shows.overview}</figcaption>
      ...
      let message =
        err.statusText || "Ocurrió un error inesperado al cargar los shows";
